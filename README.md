...
- **Depth recurrence** over layers 3-5 for 17 virtual layers total
- **4x MLP expansion** with **LeakyReLU(0.5)²** activation
- **Full Attention Residuals (AttnRes)** over virtual depth to stabilize recurrent passes
- **Parallel residuals** on deeper blocks to avoid later-stage bottlenecks
...
- **Mixed int6/int8 export** with late QAT adaptation
- **LZMA vs Brotli-11 artifact comparison** with the smaller compressed blob selected automatically
- **Late QAT** triggered at warmdown threshold 0.15

### Weight Averaging
- **EMA(0.997)** with decay scheduling -- replaces SWA for smoother convergence
- **EMA-only default path** for short 10-minute runs
...
- SGD(lr=0.004, momentum=0.9), 3 epochs, cosine LR decay, grad clip 1.0
...
- Both remain disabled in the default hybrid path until ablations justify enabling them
...
pip install mlx numpy sentencepiece huggingface-hub datasets tqdm brotli
```
